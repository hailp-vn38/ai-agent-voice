...
- Sentence Segmenter is a pure SpeechOutput delivery policy. Terminal punctuation `. ! ? 。！？` flushes a complete sentence immediately; decimal/version dots remain protected. Soft punctuation never flushes, and EOF submits the final remainder. `max_chars` is only an emergency bound on unfinished text, not a normal split point. Legacy `min_chars` and `soft_break_min_chars` remain accepted in configuration but do not decide delivery. SpeechOutput preserves display text for one `llm` control per segment and separately sanitizes the text admitted to TTS.
- `pending_segments` defaults to 8 and is a hard semantic queue bound distinct from the worker command queue. Full pending capacity pauses actor reads from the bounded LLM event route while it retains at most one partially processed delta; reads resume as TTS consumes segments. It never drops, overwrites or skips a Speech Segment. An unfinished sentence beyond the emergency text bound still fails `speech_output_backpressure`.
