...
1. client gửi `abort` hoặc speech mới.
2. assert `tts:stop`.
3. assert không nhận thêm packet generation 1.
4. turn 2 xử lý bình thường.
5. đưa packet generation 1 vào outbound queue trước abort; assert writer drop packet đó trước `tts:stop`.
